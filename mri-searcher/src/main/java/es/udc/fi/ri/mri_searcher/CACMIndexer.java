...
public class CACMIndexer {
	...
	public static final FieldType TYPE_W = new FieldType();
	static final IndexOptions options = IndexOptions.DOCS_AND_FREQS_AND_POSITIONS;
	
	static {
		TYPE_W.setIndexOptions(options);
		TYPE_W.setTokenized(true);
		TYPE_W.setStored(true);
		TYPE_W.setStoreTermVectors(true);
		TYPE_W.setStoreTermVectorPositions(true);
		TYPE_W.freeze();
	}
	...
	public static void setSimilarity(IndexWriterConfig config, String ir_model, float parameter) {
		switch (ir_model) {
		case "jm":
			config.setSimilarity(new LMJelinekMercerSimilarity(parameter));
			break;
		case "dir":
			config.setSimilarity(new LMDirichletSimilarity(parameter));
			break;
		}
	}
	
	
	private static boolean checkFileName(Path path) {
	...
	private static void indexDoc(IndexWriter writer, Path path, String hostname) {
		...
		try (InputStream stream = Files.newInputStream(path)) {
			...
			for (List<String> document : documents) {
				...
				field = document.get(i++);
				doc.add(new TextField("K", field, Field.Store.YES));
				field = document.get(i++);
				doc.add(new TextField("C", field, Field.Store.YES));
	...
	//FUNCION PRINCIPAL
	public static void index(String indexPath, Path docDir, String openMode, String ir_model, float parameter) {
		Directory dir = null;
	...
	public static void main(String args[]) throws IOException {
		...
		for (int i = 0; i < args.length; i++) {
			...
			case "-openmode":
				openMode = args[i + 1];
				System.out.println("++ El open mode será " + openMode); 
				i++;
				break;
			case "-index":
				option = args[i];
			...
			case "-indexingmodel":
				...
				ir_model = args[i + 1];
				parameter = args[i + 2];
				System.out.println("++ El modelo de RI para la busqueda es " + ir_model + " con suavizacion " + parameter + "\n");
				i +=2 ;
				break;			
			}
		}
		...
		switch (option) {
		case "-index":
			index(indexPath, docDir, openMode, ir_model, Float.parseFloat(parameter));
			break;
		}

		Date end = new Date();

		System.out.println("\nTotal indexing time: " + (end.getTime() - start.getTime()) + " milliseconds");
	}
}
